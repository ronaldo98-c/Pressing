...
<body class="theme-indigo">
    ...
    <section class="content">
        <div class="container-fluid">
            <h3 class="font-italic col-blue">Detail sur le vêtement</h3>
            <p>Voici les informations sur le vêtement.</p>
                        ...
                        <div class="body">
                            <form action="{{ route('entree-update',$entree->id) }}" method="POST">
                                ...
                                <div class="form-group">
                                    <div class="form-line">
                                        <select name="type" id="type" name="type" class="form-control" onchange="choix()"> 
                                          <option>{{$entree->type }}</option>
                                          <option value="simple">Simple</option>
                                          <option value="parTaille">Par taille</option>
                                        </select>
                                    </div>
                                </div>
                                <label for="pu" id="pu">Prix unitaire</label>
                                <div class="form-group">
                                    <div class="form-line">
                                        <input type="number" id="pu" name="pu" value="{{ $entree->pu }}" class="form-control" >
                                    </div>
                                </div>
                                <label for="nbrVetement">Nombre vêtement </label>
                                <div class="form-group">
                                    <div class="form-line">
                                        <input type="number" id="nbrVetement" name="nbrVetement" value="{{ $entree->nbrVetement }}" class="form-control">
                                    </div>
                                </div>
                                <label for="description">Description</label>
                                <div class="form-group">
                                    <textarea name="description" id="description"  class="form-control">{{ $entree->description }}</textarea>
                                </div>
                                <label for="poids" id="poids" >Poids</label>
                                <div class="form-group">
                                    <div class="form-line">
                                        <input type="number" name="poids" value="{{ $entree->poids }}"  id="poids" class="form-control" placeholder="Entrer le poids du vetement">
                                    </div>
                                </div>
                                <label for="client" id="client" >Choix client</label>
                                <div class="form-group">
                                    <div class="form-line">
                                        <select name="client" id="client" class="form-control">
                                            <option>{{$entree->client->nom}}</option>
